...
VertexOut VS(VertexIn vin)
{
	...
	//Output vertex attributes for interpolation 
	vout.Tex = mul(float4(vin.Tex, 0.0f, 1.0f), gTexTransform).xy;

	return vout;
}

float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexture, uniform bool gAlphaClip,
	uniform bool gFogEnabled) : SV_TARGET
{
	...
	if (gUseTexture)
	{
		...
		if (gAlphaClip)
		{
			...
			//We do this as soon as possible so that we can potentially
			//exit the shader early, thereby skipping the rest of the shader code
			clip(texColor.a - 0.1);
		}
	}
