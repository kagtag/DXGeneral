	...
	mTheta(1.3f*MathHelper::Pi), mPhi(0.4f*MathHelper::Pi), mRadius(80.0f)
{
	...
	m_enable4xMsaa = false; //turn off MSAA
...
bool BlendApp::Init()
{
	...
	mWaves.Init(160.0f, 160.0f, 1.0f, 0.03f, 5.0f, 0.3f);
...
void BlendApp::UpdateScene(float dt)
{
	...
	if ((m_timer.TotalTime() - t_base) >= 1.0f)
	{
		t_base += 0.1f;
	...
	for (UINT i = 0; i < mWaves.VertexCount(); ++i)
	{
		...
		v[i].Tex.y = 0.5 - mWaves[i].z / mWaves.Depth();
	}
	...
	//Combine scale with translation
...
bool BlendApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		m_d3dImmediateContext->RSSetState(RenderStates::NoCullRS); //because we can see through the box
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		//transparent blending for water
		m_d3dImmediateContext->OMSetBlendState(RenderStates::TransparentBS, blendFactor, 0xffffffff);//blender state
		landAndWavesTech->GetPassByIndex(p)->Apply(0, m_d3dImmediateContext);
...
XMFLOAT3 BlendApp::GetHillNormal(float x, float z)const
{
	...
	DirectX::XMVECTOR unitNormal = XMVector3Normalize(XMLoadFloat3(&n));
