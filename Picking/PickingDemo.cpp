...
private:
	...
	// Kepp system memory copies of the mesh geometry for picking
...
PickingApp::PickingApp(HINSTANCE hInstance)
	...
	mPickedTriangle(-1)
{
	m_mainWndCaption = L"Picking Demo";
...
bool PickingApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		m_d3dImmediateContext->IASetVertexBuffers(0, 1, &mMeshVB, &stride, &offset);
		m_d3dImmediateContext->IASetIndexBuffer(mMeshIB, DXGI_FORMAT_R32_UINT, 0);
		...
		activeMeshTech->GetPassByIndex(p)->Apply(0, m_d3dImmediateContext);
		m_d3dImmediateContext->DrawIndexed(mMeshIndexCount, 0, 0);
		...
		if (mPickedTriangle != -1)
		{
			...
			// the depth test, This is because we redraw the picked triangle with a different material to highlight it

			m_d3dImmediateContext->OMSetDepthStencilState(RenderStates::LessEqualDSS, 0);

			Effects::BasicFX->SetMaterial(mPickedTriangleMat);
			activeMeshTech->GetPassByIndex(p)->Apply(0, m_d3dImmediateContext);
			m_d3dImmediateContext->DrawIndexed(3, 3 * mPickedTriangle, 0);
...
void PickingApp::BuildMeshGeometryBuffers()
{
	...
	HR(m_d3dDevice->CreateBuffer(&vbd, &vinitData, &mMeshVB));
	...
	HR(m_d3dDevice->CreateBuffer(&ibd, &iinitData, &mMeshIB));
}

void PickingApp::Pick(int sx, int sy) 
{
	XMFLOAT4X4 P;
	XMStoreFloat4x4(&P, mCam.Proj());
	...
	XMVECTOR rayDir = XMVectorSet(vx, vy, 1.0f, 0.0);
	...
	//
	// If we did not hit the bounding box, then it is impossible that we hit the mesh,
	// so do not waste effort doing ray/triangle tests
