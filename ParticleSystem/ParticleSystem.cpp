...
ParticleSystem::ParticleSystem()
	...
	mTexArraySRV(0), mRandomTexSRV(0)
{
	mFirstRun = true;
	mGameTime = 0.0f;
	mTimeStep = 0.0f;
...
void ParticleSystem::Draw(ID3D11DeviceContext* dc, const Camera& cam)
{
	...
	// Set IA Stage
	dc->IASetInputLayout(ParInputLayouts::Particle);
	...
	// The updated vertices are stream-out to the target VB.
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		{
			dc->DrawAuto(); // draw the geometry written to a vertex buffer with SO
		}
	}
