...
// Random texture used to genrate random numbers in shaders
	...
	inout PointStream<Particle> ptStream)
{
	...
	if (gin[0].Type == PT_EMITTER)
	{
		...
		if (gin[0].Age > 0.002f)
		{
			for (int i = 0; i < 5; ++i)
			{
				...
				float3 vRandom = 35.0f * RandVec3((float3)i / 5.0f);
				vRandom.y = 20.0f;
	...
	{
		// Specify condition to keep particles, this may vary from system to system
		if (gin[0].Age <= 3.0f)
			ptStream.Append(gin[0]);
	}
}
	...
	inout LineStream<GeoOut> lineStream)
{
	...
	if (gin[0].Type != PT_EMITTER)
	{
		// Slant line in accleration direction
