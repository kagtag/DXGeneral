...
{
	m_mainWndCaption = L"Bezier Surface Demo";
...
bool BasicTessellation::DrawScene()
{
	...
	m_d3dImmediateContext->IASetInputLayout(InputLayouts::Pos);
	m_d3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_16_CONTROL_POINT_PATCHLIST);
	...
	{
		m_d3dImmediateContext->IASetVertexBuffers(0, 1, &mQuadPatchVB, &stride, &offset);
		...
		Effects::BezierTessellationFX->TessTech->GetPassByIndex(0)->Apply(0, m_d3dImmediateContext);

		m_d3dImmediateContext->RSSetState(RenderStates::WireframeRS);
		m_d3dImmediateContext->Draw(16, 0);
	}
...
void BasicTessellation::BuildQuadPatchBuffer()
{
	...
	HR(m_d3dDevice->CreateBuffer(&vbd, &vinitData, &mQuadPatchVB));
}
