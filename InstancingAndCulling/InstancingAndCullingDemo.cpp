...
private:
	...
	// Define transformations from local sapces to world space
...
{
	m_mainWndCaption = L"Instancing and Culling Demo";
...
InstancingAndCullingApp::~InstancingAndCullingApp()
{
	...
	MyEffects::DestroyAll();
	MyInputLayouts::DestroyAll();
}
...
bool InstancingAndCullingApp::Init()
{
	...
	MyEffects::InitAll(m_d3dDevice);
	MyInputLayouts::InitAll(m_d3dDevice);
...
void InstancingAndCullingApp::UpdateScene(float dt)
{
	...
	if (mFrustumCullingEnabled)
	{
		...
		m_d3dImmediateContext->Map(mInstancedBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedData);
		...
		for (UINT i = 0; i < mInstancedData.size(); ++i)
		{
			...
			if (localspaceFrustum.Contains(mSkullBox) || localspaceFrustum.Intersects(mSkullBox))
			{
		...
		m_d3dImmediateContext->Unmap(mInstancedBuffer, 0);
	}
	else
	{
...
bool InstancingAndCullingApp::DrawScene()
{
	...
	m_d3dImmediateContext->IASetInputLayout(MyInputLayouts::InstancedBasic32);
	...
	MyEffects::InstancedBasicFX->SetDirLights(mDirLights);
	MyEffects::InstancedBasicFX->SetEyePosW(mCam.GetPosition());

	ID3DX11EffectTechnique* activeTech = MyEffects::InstancedBasicFX->Light3Tech;
	...
	{
		// Draw the skull
		m_d3dImmediateContext->IASetVertexBuffers(0, 2, vbs, stride, offset);
		m_d3dImmediateContext->IASetIndexBuffer(mSkullIB, DXGI_FORMAT_R32_UINT, 0);

		XMMATRIX world = XMLoadFloat4x4(&mSkullWorld);
		XMMATRIX worldInvTranspose = MathHelper::InverseTranspose(world);

		MyEffects::InstancedBasicFX->SetWorld(world);
		MyEffects::InstancedBasicFX->SetWorldInvTranspose(worldInvTranspose);
		MyEffects::InstancedBasicFX->SetViewProj(viewProj);
		MyEffects::InstancedBasicFX->SetMaterial(mSkullMat);

		activeTech->GetPassByIndex(p)->Apply(0, m_d3dImmediateContext);
		m_d3dImmediateContext->DrawIndexedInstanced(mSkullIndexCount, mVisibleObjectCount, 0, 0, 0);
	}
...
void InstancingAndCullingApp::BuildInstancedBuffer()
{
	...
	HR(m_d3dDevice->CreateBuffer(&vbd, 0, &mInstancedBuffer));
}