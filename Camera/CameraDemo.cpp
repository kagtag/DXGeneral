...
CameraApp::CameraApp(HINSTANCE hInstance)
	...
	mLightCount(3)
{
	m_mainWndCaption = L"Camera Demo";
...
bool CameraApp::DrawScene()
{
	...
	// skull does not have texture coordinates, so we need a seperate technique
	// for it
...
void CameraApp::BuildShapeGeometryBuffers()
{
	...
	//Chache the starting index for each object in the concatenated index buffer
	mBoxIndexOffset = 0;
	...
	UINT  totalVertexCount =
		mCylinderVertexOffset + cylinder.Vertices.size();

	UINT totalIndexCount =
		mCylinderIndexOffset + mCylinderIndexCount;

	// 
	// Extract the vertex elements we are interested in and pack the vertices of
	// all the meshes into 1 vertex buffer
	//
