...
using PackedVector::HALF; // for storing 16-bit float
...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	// Divide heightmap into patches such taht each patch has CellsPerPatch
	...
	// Build the texture2d array offline
	CreateDDSTextureFromFile(device, mInfo.LayerMapFilename0.c_str(), 0, &mLayerMapArraySRV);

	CreateDDSTextureFromFile(device, mInfo.BlendMapFilename.c_str(), 0, &mBlendMapSRV);
	...
	ID3D11InputLayout* layout, TerrainEffect* effect)
{
	...
	effect->SetEyePosW(cam.GetPosition());
	effect->SetDirLights(lights);
	
	effect->SetFogColor(Colors::Silver);
	effect->SetFogStart(15.0f);
	effect->SetFogRange(175.0f);
	
	effect->SetMinDist(20.0f);
	effect->SetMaxDist(500.0f);
	effect->SetMinTess(0.0f);
	effect->SetMaxTess(6.0f);

	effect->SetTexelCellSpaceU(1.0f / mInfo.HeightmapWidth);
	effect->SetTexelCellSpaceV(1.0f / mInfo.HeightmapHeight);
	effect->SetWorldCellSpace(mInfo.CellSpacing);
	effect->SetWorldFrustumPlanes(worldPlanes);

	effect->SetLayerMapArray(mLayerMapArraySRV);
	effect->SetBlendMap(mBlendMapSRV);
	effect->SetHeightMap(mHeightMapSRV);

	effect->SetMaterial(mMat);

	ID3DX11EffectTechnique* tech = effect->Light1Tech;
	...
	// FX sets tessellation states, but it does not disable them, so do
	// that here to turn off tessellation 
...
{
	// Function computes the average heightof the ij elemnt.
	// It averages itself with its 8 neighbor pixels. Note
	// that if a pixel is missing neighbor, we just don't include
	// it in the average, that is, edge pixels don't have a neighbor pixel.
	//
	// ----------
	// | 1| 2| 3|
	// ----------
	...
	// Use int to allow negatives. If we use UINT, at i = 0, m = i-1 = UINT_MAX 
