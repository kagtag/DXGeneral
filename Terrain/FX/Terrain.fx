...
VertexOut VS(VertexIn vin)
{
	...
	// Displace the patch corners to world space. This is to
	// make the eye to patch distance calculation more accurate
	vout.PosW.y = gHeightMap.SampleLevel(samHeightmap, vin.Tex, 0).r;

	// Output vertex attributes to next state
	vout.Tex = vin.Tex;
...
// Returns true if the box is completely behind the plane (in the negative half space)
bool AabbBehindPlaneTest(float3 center, float3 extents, float4 plane)
{
	float3 n = abs(plane.xyz);

	// This is always positive
	float r = dot(extents, n); // projected radius

	// signed distance from center point to plane
	float s = dot(float4(center, 1.0f), plane);

	// if the center point of the box is a distance r or more behind the
	// plane, ( signed distance s < -r ), then the box is completely in
...
PatchTess ConstantHS(InputPatch<VertexOut, 4> patch, uint patchID : SV_PrimitiveID)
{
	...
	// Get the patch BoundsY from the first control point (top-left)
	...
	{
		// if not visible, skip the tessellation stage
		pt.EdgeTess[0] = 0.0f;
		pt.EdgeTess[1] = 0.0f;
		pt.EdgeTess[2] = 0.0f;
		pt.EdgeTess[3] = 0.0f;
	...
	{
		// Normal tessellation based on distance.
		...
		// Compute the midpoint on edges, and patch center
...
{
	// Estimate normal and tangent using central difference.
	float2 leftTex = pin.Tex + float2(-gTexelCellSpaceU, 0.0f);
	float2 rightTex = pin.Tex + float2(gTexelCellSpaceU, 0.0f);

	float2 bottomTex = pin.Tex + float2(0.0f, gTexelCellSpaceU);
	float2 topTex = pin.Tex + float2(0.0f, -gTexelCellSpaceU);
	...
	float3 bitan = normalize(float3(0.0f, bottomY - topY, -2.0f*gWorldCellSpace)); // negative z-direction
	...
	// Sample layers in the texture array
